...
def geometry_to_points(geo):
    ...
    points['seat tube top']    = a([
        - cos(radians(geo['st angle'])) * geo['st length'],
        sin(radians(geo['st angle'])) * geo['st length'],
    ])

    # Axles
    points['rear axle'] = a([
        -sqrt(geo['cs length']**2 - geo['bb drop']),
        geo['bb drop'],
    ])
    points['front axle'] = a([
    ...
    points['head tube bottom'] = a([
        cos(radians(geo['ht angle'])) * geo['ht length'] + geo['reach'],
        - sin(radians(geo['ht angle'])) * geo['ht length'] + geo['stack'],
    ])
